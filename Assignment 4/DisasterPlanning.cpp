 ...
 */
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork, const Set<string>& uncoveredCities,
                               int numCities,
    ...
    string city1 = uncoveredCities.first();
    Set<string> city1_neighbor = roadNetwork[city1];
    supplyLocations += city1;

    if (canBeMadeDisasterReadyRec(roadNetwork, uncoveredCities-city1-city1_neighbor, numCities-1, supplyLocations)) {
        return true;
    } else {
        supplyLocations -= city1;
        for (const string& city2 : city1_neighbor) {
            supplyLocations += city2;
            Set<string> city2_neighbor = roadNetwork[city2];
            if (canBeMadeDisasterReadyRec(roadNetwork, uncoveredCities-city2-city2_neighbor, numCities-1, supplyLocations)) {
                return true;
            }
            supplyLocations -= city2;
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
     ...
     */
    if (numCities < 0) {
        error("numCities shouldn't be negative.");
    }
    ...
    return  canBeMadeDisasterReadyRec(roadNetwork, uncoveredCities, numCities, supplyLocations);
}
