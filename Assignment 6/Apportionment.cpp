...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if (numSeats < populations.size()) {
        error("The Num of seats is unenough.");
    }
    ...
    for (const string& state : populations) {
        ans[state] = 1;
        hq.enqueue({state, -populations[state]/sqrt(2)});
    }
    numSeats -= populations.size();

    while (numSeats > 0) {
        DataPoint cur = hq.dequeue();
        ans[cur.name]++;
        double newWeight = -populations[cur.name]/sqrt(ans[cur.name] * (ans[cur.name]+1));
        hq.enqueue({cur.name, newWeight});
