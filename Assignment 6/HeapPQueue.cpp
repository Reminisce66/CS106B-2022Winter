...
void HeapPQueue::resize() {
    ...
    int newallocatedSize = allocatedSize * 2;
    DataPoint* newelems = new DataPoint[newallocatedSize];
...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    int child = logicalSize;

    for (int parent = logicalSize/2; parent >= 1; parent /= 2) {
...
DataPoint HeapPQueue::dequeue() {
    ...
    for (int parent = 1; parent <= logicalSize/2;) {
        int tmp;
        if (logicalSize >= 2*parent + 1) {
            tmp = elems[parent*2].weight < elems[parent*2+1].weight ? parent*2 : parent*2+1;
        } else {
            tmp = parent*2;
        }
        if (elems[tmp].weight < elems[parent].weight) {
            swap(elems[tmp], elems[parent]);
            parent = tmp;
        } else {
            break;
        }
