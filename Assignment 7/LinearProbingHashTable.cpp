...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    while (elems[value].type == SlotType::FILLED) {
        value++;
        if (value >= hashFn.numSlots()) value = 0;
    }
    elems[value].value = elem;
    elems[value].type = SlotType::FILLED;
    usedSize++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int value = hashFn(elem);
    int times = 0;
    while (elems[value].type != SlotType::EMPTY && times<hashFn.numSlots()) {
        if (elems[value].type != SlotType::TOMBSTONE && elems[value].value == elem) {
            return true;
        }
        value++;
        if (value >= hashFn.numSlots()) value = 0;
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    while (elems[value].type != SlotType::EMPTY && times<hashFn.numSlots()) {
        if (elems[value].type != SlotType::TOMBSTONE && elems[value].value == elem) {
            elems[value].type = SlotType::TOMBSTONE;
            usedSize--;
            return true;
        }
        value++;
