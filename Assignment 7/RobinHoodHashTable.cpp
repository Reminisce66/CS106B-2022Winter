...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    int value = hashFn(elem);
    Slot cur = {elem, 0};
    while (elems[value].distance != EMPTY_SLOT) {
        if (cur.distance > elems[value].distance) {
            Slot tmp = elems[value];
            elems[value] = cur;
            cur = tmp;
        }
        cur.distance++;
        value++;
        if (value >= hashFn.numSlots()) value = 0;
    }
    elems[value] = cur;
    usedSize++;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int cur_distance = 0;
    while (elems[value].distance != EMPTY_SLOT) {
        if (cur_distance > elems[value].distance) {
            return false;
        }
        if (elems[value].value == elem) {
            return true;
        }
        value++;
        if (value >= hashFn.numSlots()) value = 0;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    if (elems[value].distance == EMPTY_SLOT) return false;
    elems[value].distance = EMPTY_SLOT;

    value++;
    if (value >= hashFn.numSlots()) value = 0;

    while (elems[value].distance != EMPTY_SLOT && elems[value].distance != 0) {
        int front = value > 0 ? value-1 : hashFn.numSlots()-1;
        swap(elems[front], elems[value]);
        elems[front].distance--;
        value++;
        if (value >= hashFn.numSlots()) value = 0;
    }
    usedSize--;
